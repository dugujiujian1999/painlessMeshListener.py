...
class Node:
    ...
    async def init_connection(self, reader: StreamReader, writer: StreamWriter):
        ...
        try:
            while not reader.at_eof():
                ...
                if from_id == 0 and json_data["type"] in [PackageType.TIME_SYNC, PackageType.NODE_SYNC_REQUEST, PackageType.NODE_SYNC_REPLY]:
                    from_id = json_data["from"]
                    self.connections[from_id] = Connection(writer, from_id)

                if from_id in self.connections:
                    self.connections[from_id].last_received = asyncio.get_running_loop().time()
    ...
    async def handle_message(self, from_id, json_data):
        if from_id not in self.connections:
            ...
            return False
        if json_data["type"] in [PackageType.NODE_SYNC_REQUEST, PackageType.NODE_SYNC_REPLY]:
            self.connections[from_id].subs = [SubConnection(**sub) for sub in json_data["subs"]]
    ...
    async def send_node_sync(self, from_id, request=True):
        if from_id in self.connections:
            pack = {
                "dest": from_id,
                "from": self.node_id,
                "type": PackageType.NODE_SYNC_REQUEST if request else PackageType.NODE_SYNC_REPLY,
                "subs": [{"nodeId": v.node_id, "subs": v.subs} for k, v in self.connections.items() if k != from_id]
            }
            await self.safe_write(from_id, json.dumps(pack))
        else:
